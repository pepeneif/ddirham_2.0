...
#if defined(NDEBUG)
# error "DDirham cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x22c525cef97aeae260037ae4c0d4f80950ef84cc4f8afa63227018b0edbbbe75");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DDirham: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DDirham Signed Message:\n";
...
{
    // DDirham: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 1000); // DDirham: 20k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // DDirham: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // DDirham: 30 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DDirham: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DDirham: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x22c525cef97aeae260037ae4c0d4f80950ef84cc4f8afa63227018b0edbbbe75");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3 de Diciembre del 2014, Plantean Apertura de los sindicatos, Reforma, Nacional";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1417607953;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2712260;
        ...
        assert(block.hashMerkleRoot == uint256("0x70f7df0f80b6c4d502c0ea316d45866e403202a1c668e3be578f51fbdac43dce"));

// ------------- Code to generate Genesis Block -------------- //
...
// ------------- End of code to generate Genesis Block ----------- //
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // DDirham: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DDirhamMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DDirhamMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DDirhamMiner(CWallet *pwallet)
{
    printf("DDirhamMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ddirham-miner");
    ...
    try { loop {
        ...
        printf("Running DDirhamMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DDirhamMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DDirhamMiner, pwallet));
}
